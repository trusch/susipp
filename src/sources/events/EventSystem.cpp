...
static Susi::EventSystem eventSystem;
...
long Susi::subscribe(std::string topic,std::function<void(Event&)> callback){
	if (testIfGlob(topic)){
		return eventSystem.subscribe_glob(topic,callback);
	}
	return eventSystem.subscribe(topic,callback);
}
...
void Susi::unsubscribe(std::string topic,long id){
	if (testIfGlob(topic)){
		eventSystem.unsubscribe_glob(topic,id);
	}
